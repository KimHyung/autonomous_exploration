...
class Frontiers
{
    ...
    public:
        ...
        void cluster_frontier(){
            ...
            // double result;
            // start = clock();
            ROS_ERROR("2-1. before BFS");
            for(int i=0; i<frontier.size(); i++){
                bfs_search(frontier[i]);
                group_id++;
            }
            ROS_ERROR("2-2. after BFS");
            // end = clock();
            // result = (double)(end - start);
        ...
        void bfs_search(int index){
            ...
            q.push(index);
            visited[index] = 1;
            ROS_ERROR("in (%d, %d)",visited.size(), index);
            clustered[group_id].id = group_id;
            ROS_ERROR("2 out");
            while(!q.empty()){
                // clustered[group_id].frontier_index.push_back(index);
                // ROS_ERROR("2-2. before(%d,%d,  %d,%d,  %d)",x,y,map->info.width,map->info.height,q.size());
                x = q.front() % map->info.width;
                y = q.front() / map->info.height;
                q.pop();
                //4direction-check
                for(int i=0; i<8; i++){
                    int nx = x+WAYS[i][0];
                    ...
                    if(nx <= map->info.width-1 && nx >= 0 && ny <= map->info.height-1 && ny >= 0){
                        next = gridTomap(nx, ny, map->info.height);
        ...
        void publish_frontier(){
            ...
            for(int i=0; i<frontier.size();i++){
                ...
                int t_y = frontier[i] / map->info.height;
            ...
            for(int j=0; j<group_id; j++){
                ...
                if(num_frontier>10){
                    ...
                    for(int i=0;i<clustered[j].frontier_index.size();i++){
                        ...
                        int t_y = clustered[j].frontier_index[i] / map->info.height;
                    ...
                    int t_y = clustered[j].frontier_index[num_frontier/2] / map->info.height;
        ...
        int cal_dist_factor(){
            ...
            if(make_plan_){
                ...
                for(int i=0; i<valid_cluster.size();i++){
                    ...
                    int t_y = clustered[valid_cluster[i]].frontier_index[num_frontier/2] / map->info.height;
        ...
        void next_best_view(){
            ...
            if(!segment_img.empty() && regions.size() != 0){
                ...
                for(int i=0; i<valid_cluster.size();i++){
                    ...
                    int t_y = clustered[valid_cluster[i]].frontier_index[clustered[valid_cluster[i]].frontier_index.size()/2] / map->info.height;
                    double include_region = regions[segment_img.at<uchar>(t_y,t_x)];
                    for(int j=0;j<4;j++){
                        int nx = t_x + dx[j];
                        int ny = t_y + dx[j];
                        if(map->data[gridTomap(nx,ny,map->info.height)]==0){
                            printf("%d\n",map->data[gridTomap(nx,ny,map->info.height)]);
                    ...
                    // clustered[valid_cluster[i]].region_factor = (robot_region == include_region)?0.5:0;
                    // double obj = (3*exp(-((double)clustered[valid_cluster[i]].dist_factor / (double)total_waypoints)) + exp(clustered[valid_cluster[i]].num_factor))/2 + clustered[valid_cluster[i]].region_factor;
                    ...
                    // double obj = (3*exp((double)-clustered[valid_cluster[i]].dist_factor / total_waypoints) + exp(clustered[valid_cluster[i]].num_factor))/2;
                ...
                if(valid_cluster.size()!= 0){
                    ...
                    int t_y = clustered[answer].frontier_index[clustered[answer].frontier_index.size()/2] / map->info.height;
        ...
        //utils
        int gridTomap(int x, int y, int height){
            return y * height + x;
        }

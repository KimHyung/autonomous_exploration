...
#include "plane_detector_ros.hpp"
#include "ros/ros.h"
...
class FloorDetector:public PlaneDetectorROS {
public:
...
FloorDetector::FloorDetector(ros::NodeHandle &nh, ros::NodeHandle &pnh): PlaneDetectorROS(nh, pnh), pcd_mod(NULL),publish_cloud(true)
{
  // Set defaults
  ...
  img_topic_1 = cam_1 + "/depth/image_raw";
  info_topic_1 = cam_1 + "/depth/camera_info";
  pc_topic = nh.resolveName("/out_cloud");
  
  
  while (!getTransformFromTF() && ros::ok()) {
    sleep(1);
  }
...
FloorDetector::FloorDetector(double delta, double epsilon, double gamma, int theta, const std::string &cam, const Eigen::Affine3d& T): PlaneDetectorROS(delta, epsilon, gamma, theta), T(T)
{
...
void FloorDetector::detectFloor(const sensor_msgs::Image &img)
{
  ...
  for (unsigned int i = 0; i < _detected_planes.size() ; i++) {
    ...
    if (fabs(p.v.dot(v_z)) > 0.75 && p.d < floor_tolerance) {
  ...
  //     std::cout  << i << std::endl;
	...
	for (unsigned int j = 0; j < floor_planes.size(); j++) {
	  if ( _detected_planes.at(floor_planes.at(j)).distance(p) < floor_tolerance ) {
	    ...
	    *iter_y = p(1);
	    *iter_z = p(2);
	    int c = _status_vec.at(i)%_color.size();
	    *iter_r = _color[c](0);
	    *iter_g = _color[c](1);
	    *iter_b = _color[c](2);        
